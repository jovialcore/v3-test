...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const QUANTITY_PAGES = computed(() => {
      return Math.round(props.totalItems/props.maxItems);
    });
    const BACK_DISABLED = computed(() => {
      return props.startItems === (((props.startItems * props.maxItems) - props.maxItems) + 1);
    });
    const NEXT_DISABLED = computed(() => {
      return props.startItems === (((props.maxItems * QUANTITY_PAGES.value) - props.maxItems) + 1);
    });

    const PAGE_SELECTED = (number: number) => {
      return props.startItems == (((number * props.maxItems) - props.maxItems) + 1);
    };
